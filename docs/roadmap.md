...
- [ ] **Grafana dashboard reading the OTLP data (Axis 1)** — a Grafana dashboard (JSON model + provisioning) that reads the metrics/traces exported above. Realizes Axis 1 of the observability map (`docs/kubeverdict-patchtst-map.md`), generalized from a Jaeger-only live trace backend to OTLP-native self-monitoring of kube-verdict itself, ahead of a real GCP / sovereign-cloud deployment (needs a real OTel Collector + Grafana instance — see `docs/cloud-prerequisites.md`). Distinct from PatchTST's own validation dashboard (Axis 3, `tools/render_dashboard.py`), which stays in the PatchTST repo — no duplication across the two systems.
