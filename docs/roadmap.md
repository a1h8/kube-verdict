...
- [x] **More h-series cases (h013+)** — all three named cases landed, one per observability signal, each through the real collector: **h013** SLO error-budget burn (p99/p95 latency-SLO breach + multi-window burn-rate alerts on a pod Kubernetes reports healthy; `prometheus/` fixtures → `PrometheusCollector`), **h014** expired mTLS certificate (cert-manager issuer cannot renew, pods Running but not ready, cause only in logs; `loki/` fixtures → `LokiSource`) and **h015** etcd compaction latency (cause visible only in OTel error traces; `otel/` fixtures → `OtelCollector`). They prove the fixture → graph → context-window path deterministically; LLM root-cause text still needs Ollama, and no live Loki/OTel capture exists yet. Not to be confused with B10's "log-first RCA" item, which stays open (its check is a case named for Loki, and it asks for the hypothesis path end to end).
...
## Showcase — Unified Vitrine (B15)

The 3 dashboards exist separately today (Score, Decision Journey, and Grafana/OTLP once
built) but don't tell a story together. B15 links them into one narrative vitrine: alert →
reasoning → verdict → service health — built for a demo/pitch audience, not just internal
debugging.

- [x] **Evidence panel (logs/traces/metrics) in Decision Journey** — `hypothesis_sources` (`rule_id`, `symptom`, `affected`, `weight`, `evidence`) was already returned by `GET /sessions/{id}/state` but never rendered; `HypothesisEvidencePanel` (`dashboard/src/DecisionJourney.jsx`) now shows the concrete evidence lines behind each hypothesis, not just the reasoning text. Loki stays log-only — traces always come from OTel/Tempo, never Loki.
- [x] **Self-observability instrumentation (OTLP export)** — `telemetry.py` (root module, not under `api/` — both `api/app.py` and `workflow/nodes.py`/`rca/analyzer.py` import it) wires the OpenTelemetry SDK behind `OTEL_SELF_MONITORING_ENABLED` (no-op otherwise): `FastAPIInstrumentor` auto-captures request latency/error rate per route as traces; `kubeverdict.llm.call.duration` histogram times both LLM call sites (`workflow/nodes.py` hypothesize, `rca/analyzer.py` analyze); `kubeverdict.collector.fallback` counter increments from the existing `_stats()` choke point whenever a collector reports `fallback=True`. Exports OTLP push to `OTEL_EXPORTER_OTLP_ENDPOINT`. Covered by `tests/unit/test_telemetry.py`.
- [x] **Grafana dashboard reading the OTLP data (Axis 1)** — JSON model (`helm/kube-verdict/dashboards/monitoring-ops.json`) + Helm provisioning (`templates/grafana-dashboard.yaml`, a ConfigMap labeled `grafana_dashboard: "1"` — `demo/observability/grafana-values.yaml` sidecar set to `searchNamespace: ALL` since Grafana lives in `kubeverdict-obs`, this release in `kubeverdict`). Still needs `OTEL_SELF_MONITORING_ENABLED=true` on the live release and a rendered-panel check once the cluster is back up — see the checklist in `docs/cloud-prerequisites.md`. Reads the metrics/traces exported above. Realizes Axis 1 of the observability map (`docs/kubeverdict-patchtst-map.md`), generalized from a Jaeger-only live trace backend to OTLP-native self-monitoring of kube-verdict itself, ahead of a real GCP / sovereign-cloud deployment (needs a real OTel Collector + Grafana instance — see `docs/cloud-prerequisites.md`). Distinct from PatchTST's own validation dashboard (Axis 3, `tools/render_dashboard.py`), which stays in the PatchTST repo — no duplication across the two systems.
- [ ] **Vitrine narrative** — a landing view in `dashboard/` that chains the 3 dashboards as a guided story instead of 3 isolated links: alert fires → Decision Journey (reasoning + evidence) → verdict → link into the Grafana/OTLP dashboard (service health). Built for a demo, not just internal navigation.

Being demonstrable also means being demonstrable on real infra, not just fixtures — see
`docs/cloud-prerequisites.md` for the GCP/Scaleway checklist (accounts, real Prometheus/Loki/OTel
backends for h013/h014/h015, the Grafana/OTLP dashboard live on both, and a recorded proof run).
