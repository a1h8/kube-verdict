...
- [x] **More h-series cases (h013+)** — landed: **h013** SLO error-budget burn (p99/p95 latency-SLO breach + multi-window burn-rate alerts on a pod Kubernetes reports healthy, evidence via `prometheus/` fixtures through the real `PrometheusCollector`) and **h015** etcd compaction latency (cause visible only in OTel error traces, via `otel/` fixtures through the real `OtelCollector`). Both prove the fixture → graph → context-window path deterministically; LLM root-cause text still needs Ollama. **Still open: h014 (cert expiry).** The check turns green once any `h013+` case exists, so this box stays ticked while h014 remains.
