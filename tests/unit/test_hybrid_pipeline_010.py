"""
Pipeline tracing test — case 010 Readiness Probe Failure (no Ollama required).

Verifies the full hybrid retrieval chain AND dry-run resolution path for the
order-service readiness probe failure scenario: DB_POOL_SIZE drift (20 declared
→ 2 observed) exhausts the connection pool, causing readiness HTTP 500.
...
CASE_DIR = Path(__file__).parent.parent.parent / "cases" / "010_readiness_probe_failure"
QUERY    = "order-service pod Running but not Ready, service has no endpoints"
...
class TestStep1Tokenizer:
    ...
    def test_query_tokens_include_readiness(self):
        tokens = _tokenize(QUERY)
        assert "ready" in tokens or "readiness" in tokens or "running" in tokens, (
            f"'ready'/'readiness'/'running' not found in {tokens}"
        )

    def test_query_tokens_include_order_service(self):
        tokens = _tokenize(QUERY)
        assert "order-service" in tokens or "order" in tokens

    def test_compound_split_preserved(self):
        # "reason=Unhealthy" → both compound AND parts indexed
        tokens = _tokenize("reason=Unhealthy probe=readiness env.DB_POOL_SIZE=2")
        assert "unhealthy" in tokens or "reason=unhealthy" in tokens
        assert "readiness" in tokens or "probe=readiness" in tokens
        assert "db_pool_size" in tokens or "env.db_pool_size=2" in tokens
...
class TestStep3SparseRetrieval:
    ...
    def test_bm25_finds_readiness_probe(self, store):
        hits = store._bm25.search(QUERY, top_k=5)
        assert hits, "BM25 returned no hits for readiness probe query"
    ...
    def test_bm25_top_hit_mentions_relevant_token(self, store):
        ...
        assert (
            "readiness" in texts_lower
            or "probe" in texts_lower
            or "order-service" in texts_lower
            or "ready" in texts_lower
            or "db_pool_size" in texts_lower
        ), (
            "Top-3 BM25 hits don't contain readiness/probe/order-service/ready/db_pool_size:\n"
...
class TestStep4RRFFusion:
    ...
    def test_rrf_fuse_boosts_agreement(self):
        ...
        shared_uid = "pod-production-order-service-6c9d8b7f4-vp5r1"
        only_dense = "event-Unhealthy-only"
...
class TestStep5ContextWindow:
    ...
    def test_seeds_contain_unhealthy_pod(self, ctx):
        ...
        for s in ctx.seeds:
            print(f"    {s[:140]}")
        assert "readiness" in seeds_text or "probe" in seeds_text or "order-service" in seeds_text, (
            f"Seeds don't mention readiness/probe/order-service:\n{ctx.seeds}"
        )
    ...
    def test_events_surfaced(self, ctx):
        ...
        for e in ctx.events[:3]:
            print(f"    {e[:140]}")
        assert (
            "unhealthy" in events_text
            or "readiness" in events_text
            or "probe" in events_text
            or "database" in events_text
        ), (
            "Expected Unhealthy or readiness probe or database in events"
        )

    def test_events_sorted_by_count_desc(self, ctx):
        """Event with count=42 should come before lower counts if both present."""
        events_text = " ".join(ctx.events)
        if "count=42" in events_text and "count=1" in events_text:
            idx_42 = next(i for i, e in enumerate(ctx.events) if "count=42" in e)
            idx_1  = next(i for i, e in enumerate(ctx.events) if "count=1" in e)
            assert idx_42 < idx_1, (
                f"Event count=42 should precede count=1 (got {idx_42} vs {idx_1})"
            )
...
class TestStep6E2ERecall:
    ...
    def test_readiness_signal_surfaced(self, ctx):
        """
        The DB_POOL_SIZE drift or readiness probe failure must appear somewhere
        in the context — events and anchors should carry it.
        """
        all_text = (
            " ".join(ctx.seeds)
            + " ".join(ctx.events)
        ...
        print("\n  [e2e recall] checking 'readiness' or 'probe' in combined context...")
        assert (
            "readiness" in all_text
            or "probe" in all_text
            or "db_pool_size" in all_text
            or "pool" in all_text
        ), (
            "Expected 'readiness', 'probe', 'db_pool_size', or 'pool' to appear in "
            "seeds/events/related — the readiness probe signal was lost"
        )
...
class TestStep7RemediationEngine:
    ...
    def test_at_least_one_hypothesis(self, hypotheses):
        assert hypotheses, "RemediationEngine produced no hypotheses for case 010"

    def test_helm_drift_rule_fires(self, hypotheses):
        """helm_drift rule must fire — DB_POOL_SIZE drift (20 → 2) is present."""
        rule_ids = [h.rule_id for h in hypotheses]
        assert "helm_drift" in rule_ids, (
            f"helm_drift rule did not fire. Fired rules: {rule_ids}"
        )

    def test_helm_drift_commands_contain_helm_upgrade(self, hypotheses):
        h = next(h for h in hypotheses if h.rule_id == "helm_drift")
        cmds_text = " ".join(h.commands).lower()
        assert "helm" in cmds_text and "upgrade" in cmds_text, (
            f"helm_drift commands don't include 'helm upgrade': {h.commands}"
        )

    def test_helm_drift_commands_reference_namespace(self, hypotheses):
        h = next(h for h in hypotheses if h.rule_id == "helm_drift")
    ...
    def test_top_hypothesis_commands_contain_kubectl(self, hypotheses):
        """The top hypothesis must include a useful command."""
        top = hypotheses[0]
        cmds_text = " ".join(top.commands).lower()
        assert "kubectl" in cmds_text or "helm" in cmds_text, (
            f"Top hypothesis commands don't include kubectl or helm: {top.commands}"
        )
...
class TestStep8PromptDryRun:
    ...
    def test_prompt_contains_readiness_or_probe(self, prompt):
        assert "readiness" in prompt.lower() or "probe" in prompt.lower() or "unhealthy" in prompt.lower()

    def test_prompt_contains_db_pool_signal(self, prompt):
        """The DB_POOL_SIZE drift must be visible to the LLM."""
        assert (
            "db_pool_size" in prompt.lower()
            or "pool" in prompt.lower()
            or "database" in prompt.lower()
        ), (
            "DB_POOL_SIZE/pool signal missing from prompt — LLM won't see the root cause"
        )

    def test_prompt_contains_anchor_info(self, prompt):
        """Declared anchors (readinessProbe + DB_POOL_SIZE) must appear."""
        assert "anchor" in prompt.lower() or "declared" in prompt.lower() or "db_pool" in prompt.lower(), (
...
class TestStep9Proposals:
    ...
    def dry_run_report(self, graph, ctx):
        ...
        if top:
            raw = (
                ...
                f"### 3. Root cause\n{top.explanation}\n\n"
                f"### 4. Causal chain\n"
                + "\n".join(f"{i+1}. {ev}" for i, ev in enumerate(top.evidence or ["DB_POOL_SIZE drift"]))
                + "\n\n### 5. Remediation\n"
    ...
    def test_config_or_probe_proposal_present(self, proposals):
        """At least one actionable proposal for config/drift/probe case."""
        categories = [p.category for p in proposals]
        all_text = " ".join(
            p.description + " " + p.follow_up_query for p in proposals
        ).lower()
        assert len(proposals) >= 1 and (
            "config" in categories
            or "remediation" in categories
            or "drift" in categories
            or "generic" in categories
            or "helm" in all_text
            or "probe" in all_text
        ), (
            f"Expected at least one actionable proposal. Got categories: {categories}"
        )

    def test_generic_logs_proposal_present(self, proposals):
        """At least one diagnostic/generic proposal must fire."""
        categories = [p.category for p in proposals]
        all_desc = " ".join(p.description + " " + p.follow_up_query for p in proposals).lower()
        assert (
            "generic" in categories
            or "remediation" in categories
            or "log" in all_desc
            or "kubectl" in all_desc
            or "drift" in all_desc
        ), (
            f"No diagnostic proposal found.\nProposals: {[(p.category, p.description) for p in proposals]}"
        )
