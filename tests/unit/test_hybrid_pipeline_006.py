"""
Pipeline tracing test — case 006 Pending Insufficient CPU (no Ollama required).

Verifies the full hybrid retrieval chain AND dry-run resolution path for the
api-gateway pod Pending scenario: CPU request drift (500m declared → 4000m
observed) causes the pod to be unschedulable on all 3 nodes.
...
CASE_DIR = Path(__file__).parent.parent.parent / "cases" / "006_pending_insufficient_cpu"
QUERY    = "api-gateway pod is stuck Pending, never gets scheduled"
...
class TestStep1Tokenizer:
    ...
    def test_query_tokens_include_api_gateway(self):
        tokens = _tokenize(QUERY)
        assert "api-gateway" in tokens or "api" in tokens

    def test_compound_split_preserved(self):
        # "reason=FailedScheduling" → both compound AND parts indexed
        tokens = _tokenize("reason=FailedScheduling cpu=4000m requests.cpu=500m")
        assert "failedscheduling" in tokens or "reason=failedscheduling" in tokens
        assert "cpu=4000m" in tokens or "4000m" in tokens
        assert "requests.cpu=500m" in tokens or "500m" in tokens
...
class TestStep3SparseRetrieval:
    ...
    def test_bm25_finds_pending_cpu(self, store):
        hits = store._bm25.search(QUERY, top_k=5)
        assert hits, "BM25 returned no hits for Pending/CPU query"
    ...
    def test_bm25_top_hit_mentions_relevant_token(self, store):
        ...
        assert (
            ...
            or "cpu" in texts_lower
            or "api-gateway" in texts_lower
            or "insufficient" in texts_lower
        ), (
            "Top-3 BM25 hits don't contain pending/cpu/api-gateway/insufficient:\n"
...
class TestStep4RRFFusion:
    ...
    def test_rrf_fuse_boosts_agreement(self):
        ...
        shared_uid = "pod-production-api-gateway-7f9d8c6b5-xk2p4"
        only_dense = "event-FailedScheduling-only"
...
class TestStep5ContextWindow:
    ...
    def test_seeds_contain_unhealthy_pod(self, ctx):
        ...
        for s in ctx.seeds:
            print(f"    {s[:140]}")
        assert "pending" in seeds_text or "api-gateway" in seeds_text or "cpu" in seeds_text, (
            f"Seeds don't mention pending/api-gateway/cpu:\n{ctx.seeds}"
        )
    ...
    def test_events_surfaced(self, ctx):
        ...
        assert (
            ...
            or "insufficient" in events_text
            or "cpu" in events_text
        ), (
            "Expected FailedScheduling or Insufficient cpu in events"
        )

    def test_events_sorted_by_count_desc(self, ctx):
        """Event with count=24 should come before lower counts if both present."""
        events_text = " ".join(ctx.events)
        if "count=24" in events_text and "count=1" in events_text:
            idx_24 = next(i for i, e in enumerate(ctx.events) if "count=24" in e)
            idx_1  = next(i for i, e in enumerate(ctx.events) if "count=1" in e)
            assert idx_24 < idx_1, (
                f"Event count=24 should precede count=1 (got {idx_24} vs {idx_1})"
            )
...
class TestStep6E2ERecall:
    ...
    def test_cpu_signal_surfaced(self, ctx):
        """
        The CPU drift (500m declared → 4000m observed) or Insufficient cpu event
        must appear somewhere in the context — anchors and events should carry it.
        """
        all_text = (
            " ".join(ctx.seeds)
            + " ".join(ctx.events)
        ...
        print("\n  [e2e recall] checking 'cpu' or 'insufficient' or 'pending' in combined context...")
        assert (
            "cpu" in all_text
            or "insufficient" in all_text
            or "pending" in all_text
        ), (
            "Expected 'cpu', 'insufficient', or 'pending' to appear in "
            "seeds/events/related — the CPU scheduling signal was lost"
        )
...
class TestStep7RemediationEngine:
    ...
    def test_at_least_one_hypothesis(self, hypotheses):
        assert hypotheses, "RemediationEngine produced no hypotheses for case 006"

    def test_pending_unschedulable_rule_fires(self, hypotheses):
        """pending_unschedulable rule must fire — pod is Pending with FailedScheduling event."""
    ...
    def test_helm_drift_rule_fires(self, hypotheses):
        """helm_drift rule must fire — CPU request drift (500m → 4000m) is present."""
        rule_ids = [h.rule_id for h in hypotheses]
...
class TestStep8PromptDryRun:
    ...
    def test_prompt_contains_cpu_signal(self, prompt):
        """The CPU drift / Insufficient cpu signal must be visible to the LLM."""
        assert (
            "cpu" in prompt.lower()
            or "insufficient" in prompt.lower()
        ), (
            "CPU signal missing from prompt — LLM won't see the root cause"
        )

    def test_prompt_contains_anchor_info(self, prompt):
        """Declared CPU request anchor must appear — helps LLM identify config drift."""
        assert "anchor" in prompt.lower() or "declared" in prompt.lower() or "500m" in prompt, (
...
class TestStep9Proposals:
    ...
    def dry_run_report(self, graph, ctx):
        ...
        if top:
            raw = (
                ...
                f"### 3. Root cause\n{top.explanation}\n\n"
                f"### 4. Causal chain\n"
                + "\n".join(f"{i+1}. {ev}" for i, ev in enumerate(top.evidence or ["Insufficient cpu"]))
                + "\n\n### 5. Remediation\n"
    ...
    def test_resource_or_scheduling_proposal_present(self, proposals):
        """At least one actionable proposal must be generated for the CPU/scheduling case."""
        categories = [p.category for p in proposals]
        all_text = " ".join(
            p.description + " " + p.follow_up_query for p in proposals
        ).lower()
        assert len(proposals) >= 1 and (
            "resource" in categories
    ...
    def test_generic_logs_or_events_proposal_present(self, proposals):
        """At least one generic/diagnostic proposal must fire."""
        categories = [p.category for p in proposals]
