"""
Pipeline tracing test — case 004 Pending PVC (no Ollama required).

Verifies the full hybrid retrieval chain AND dry-run resolution path for the
postgres-primary-0 Pending pod scenario: StorageClass "fast-ssd" not found,
PVC stuck Pending, ProvisioningFailed event.
...
CASE_DIR = Path(__file__).parent.parent.parent / "cases" / "004_pending_pvc"
QUERY    = "postgres-primary-0 pod is stuck in Pending state"
...
class TestStep1Tokenizer:
    ...
    def test_query_tokens_include_pending(self):
        tokens = _tokenize(QUERY)
        assert "pending" in tokens, (
            f"'pending' not found in {tokens}"
        )

    def test_query_tokens_include_postgres(self):
        tokens = _tokenize(QUERY)
        assert "postgres-primary-0" in tokens or "postgres" in tokens

    def test_compound_split_preserved(self):
        # "storageClassName=fast-ssd" → both compound AND parts indexed
        tokens = _tokenize("storageClassName=fast-ssd reason=ProvisioningFailed pvc=postgres-data")
        assert "storageclassname=fast-ssd" in tokens or "fast-ssd" in tokens
        assert "provisioningfailed" in tokens or "reason=provisioningfailed" in tokens
        assert "postgres-data" in tokens or "postgres" in tokens
...
class TestStep2DenseRetrieval:
    ...
    def test_dense_top_hit_is_pod_or_event(self, store):
        ...
        assert kinds & {"Pod", "K8sEvent", "PersistentVolumeClaim"}, (
            f"Expected Pod, K8sEvent or PVC in top-3 dense hits, got: {kinds}"
        )
...
class TestStep3SparseRetrieval:
    ...
    def test_bm25_finds_pending_pvc(self, store):
        hits = store._bm25.search(QUERY, top_k=5)
        assert hits, "BM25 returned no hits for Pending PVC query"
    ...
    def test_bm25_top_hit_mentions_relevant_token(self, store):
        ...
        assert (
            "pending" in texts_lower
            or "postgres" in texts_lower
            or "pvc" in texts_lower
            or "storageclass" in texts_lower
        ), (
            "Top-3 BM25 hits don't contain pending/postgres/pvc/storageclass:\n"
...
class TestStep4RRFFusion:
    ...
    def test_rrf_fuse_boosts_agreement(self):
        ...
        shared_uid = "pod-production-postgres-primary-0"
        only_dense = "event-ProvisioningFailed-only"
...
class TestStep5ContextWindow:
    ...
    def test_seeds_contain_unhealthy_pod(self, ctx):
        ...
        for s in ctx.seeds:
            print(f"    {s[:140]}")
        assert "pending" in seeds_text or "postgres" in seeds_text or "pvc" in seeds_text, (
            f"Seeds don't mention pending/postgres/pvc:\n{ctx.seeds}"
        )
    ...
    def test_events_surfaced(self, ctx):
        ...
        for e in ctx.events[:3]:
            print(f"    {e[:140]}")
        assert (
            "provisioningfailed" in events_text
            or "failedmount" in events_text
            or "fast-ssd" in events_text
            or "storageclass" in events_text
        ), (
            "Expected ProvisioningFailed or FailedMount or fast-ssd in events"
        )

    def test_events_sorted_by_count_desc(self, ctx):
        """Events with count=6 should come before count=1 if both present."""
        events_text = " ".join(ctx.events)
        if "count=6" in events_text and "count=1" in events_text:
            idx_6 = next(i for i, e in enumerate(ctx.events) if "count=6" in e)
            idx_1 = next(i for i, e in enumerate(ctx.events) if "count=1" in e)
            assert idx_6 < idx_1, (
                f"Event count=6 should precede count=1 (got {idx_6} vs {idx_1})"
            )
...
class TestStep6E2ERecall:
    ...
    def test_storageclass_signal_surfaced(self, ctx):
        """
        The 'fast-ssd' StorageClass not found event must appear somewhere in the
        context — BM25 should pick it up even if embedding distance is mediocre.
        """
        all_text = (
            " ".join(ctx.seeds)
            + " ".join(ctx.events)
        ...
        print("\n  [e2e recall] checking 'storageclass' or 'pvc' in combined context...")
        assert (
            "storageclass" in all_text
            or "fast-ssd" in all_text
            or "pvc" in all_text
            or "pending" in all_text
        ), (
            "Expected 'storageclass', 'fast-ssd', 'pvc', or 'pending' to appear in "
            "seeds/events/related — the StorageClass missing signal was lost"
        )
...
class TestStep7RemediationEngine:
    ...
    def test_at_least_one_hypothesis(self, hypotheses):
        assert hypotheses, "RemediationEngine produced no hypotheses for case 004"

    def test_pending_unschedulable_rule_fires(self, hypotheses):
        """pending_unschedulable rule must fire — pod is Pending with mount failure."""
        rule_ids = [h.rule_id for h in hypotheses]
        assert "pending_unschedulable" in rule_ids, (
            f"pending_unschedulable rule did not fire. Fired rules: {rule_ids}"
        )

    def test_pending_unschedulable_commands_contain_kubectl(self, hypotheses):
        h = next(h for h in hypotheses if h.rule_id == "pending_unschedulable")
        cmds_text = " ".join(h.commands).lower()
        assert "kubectl" in cmds_text

    def test_pending_unschedulable_commands_reference_namespace(self, hypotheses):
        h = next(h for h in hypotheses if h.rule_id == "pending_unschedulable")
    ...
    def test_top_hypothesis_has_kubectl_describe(self, hypotheses):
        """The top hypothesis should suggest kubectl describe for diagnosis."""
        top = hypotheses[0]
        cmds_text = " ".join(top.commands).lower()
        assert "kubectl" in cmds_text, (
            f"Top hypothesis commands don't include kubectl: {top.commands}"
        )
...
class TestStep8PromptDryRun:
    ...
    def test_prompt_contains_pending(self, prompt):
        assert "Pending" in prompt or "pending" in prompt.lower()

    def test_prompt_contains_storageclass_or_pvc_signal(self, prompt):
        """The StorageClass / PVC not found event must be visible to the LLM."""
        assert (
            "storageclass" in prompt.lower()
            or "fast-ssd" in prompt.lower()
            or "pvc" in prompt.lower()
        ), (
            "StorageClass/PVC signal missing from prompt — LLM won't see the root cause"
        )

    def test_prompt_contains_anchor_info(self, prompt):
        """Declared storageClassName anchor must appear."""
        assert "anchor" in prompt.lower() or "declared" in prompt.lower() or "fast-ssd" in prompt, (
...
class TestStep9Proposals:
    ...
    def dry_run_report(self, graph, ctx):
        ...
        if top:
            raw = (
                ...
                f"### 3. Root cause\n{top.explanation}\n\n"
                f"### 4. Causal chain\n"
                + "\n".join(f"{i+1}. {ev}" for i, ev in enumerate(top.evidence or ["PVC pending"]))
                + "\n\n### 5. Remediation\n"
    ...
    def test_storage_or_scheduling_proposal_present(self, proposals):
        """
        A storage or scheduling related proposal must appear — PVC Pending signals
        a volume provisioning issue.
        """
        ...
        assert (
            ...
            or "scheduling" in categories
            or "pvc" in all_text
            or "storageclass" in all_text
            or "pending" in all_text
            or "volume" in all_text
        ), (
            f"Expected storage/scheduling proposal (PVC check). Got categories: {categories}"
        )

    def test_generic_logs_or_events_proposal_present(self, proposals):
        """Events/describe proposal must fire — ProvisioningFailed signals."""
        all_desc = " ".join(p.description + " " + p.follow_up_query for p in proposals).lower()
        assert (
            "event" in all_desc
            or "describe" in all_desc
            or "log" in all_desc
            or "pending" in all_desc
        ), (
            f"No events/describe proposal found.\nProposals: {[(p.category, p.description) for p in proposals]}"
        )
