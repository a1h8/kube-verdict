"""
Pipeline tracing test — case 003 OOMKilled (no Ollama required).

Verifies the full hybrid retrieval chain AND dry-run resolution path for the
analytics-worker OOMKilled scenario: memory limit drift (512Mi declared → 50Mi
observed) directly explains the OOM kill.
...
CASE_DIR = Path(__file__).parent.parent.parent / "cases" / "003_oomkilled"
QUERY    = "analytics-worker keeps restarting with OOMKilled exit code 137"
...
class TestStep1Tokenizer:
    ...
    def test_query_tokens_include_oomkilled(self):
        tokens = _tokenize(QUERY)
        assert "oomkilled" in tokens, (
            f"'oomkilled' not found in {tokens}"
        )

    def test_query_tokens_include_analytics(self):
        tokens = _tokenize(QUERY)
        assert "analytics-worker" in tokens or "analytics" in tokens

    def test_compound_split_preserved(self):
        # "reason=OOMKilled" → both compound AND parts indexed
        tokens = _tokenize("reason=OOMKilled exitCode=137 memory=512Mi")
        assert "reason=oomkilled" in tokens
        assert "oomkilled" in tokens
        assert "exitcode=137" in tokens
        assert "memory=512mi" in tokens or "memory" in tokens
...
class TestStep3SparseRetrieval:
    ...
    def test_bm25_finds_oomkilled(self, store):
        hits = store._bm25.search(QUERY, top_k=5)
        assert hits, "BM25 returned no hits for OOMKilled query"
    ...
    def test_bm25_top_hit_mentions_relevant_token(self, store):
        ...
        assert "oomkilled" in texts_lower or "analytics" in texts_lower or "memory" in texts_lower, (
            "Top-3 BM25 hits don't contain OOMKilled or analytics or memory:\n"
...
class TestStep4RRFFusion:
    ...
    def test_rrf_fuse_boosts_agreement(self):
        ...
        shared_uid = "pod-production-analytics-worker-5c8f7d9b4-nm8qt"
        only_dense = "event-OOMKilling-only"
...
class TestStep5ContextWindow:
    ...
    def test_seeds_contain_unhealthy_pod(self, ctx):
        ...
        for s in ctx.seeds:
            print(f"    {s[:140]}")
        assert "analytics" in seeds_text or "oomkill" in seeds_text or "memory" in seeds_text, (
            f"Seeds don't mention analytics/OOMKill/memory:\n{ctx.seeds}"
        )
    ...
    def test_events_surfaced(self, ctx):
        ...
        for e in ctx.events[:3]:
            print(f"    {e[:140]}")
        assert "backoff" in events_text or "oom" in events_text or "memory" in events_text, (
            "Expected BackOff or OOM or memory in events"
        )

    def test_events_sorted_by_count_desc(self, ctx):
        """Events with count=8 should come before count=1 if both present."""
        events_text = " ".join(ctx.events)
        if "count=8" in events_text and "count=1" in events_text:
            idx_8 = next(i for i, e in enumerate(ctx.events) if "count=8" in e)
            idx_1 = next(i for i, e in enumerate(ctx.events) if "count=1" in e)
            assert idx_8 < idx_1, (
                f"Event count=8 should precede count=1 (got {idx_8} vs {idx_1})"
            )
...
class TestStep6E2ERecall:
    ...
    def test_memory_signal_surfaced(self, ctx):
        """
        The memory limit drift (512Mi → 50Mi) must appear somewhere in the
        context — either in events or related. Anchors should carry it.
        """
        all_text = (
            " ".join(ctx.seeds)
            + " ".join(ctx.events)
        ...
        print("\n  [e2e recall] checking 'memory' or 'limit' in combined context...")
        assert "memory" in all_text or "limit" in all_text or "oomkill" in all_text, (
            "Expected 'memory' or 'limit' or 'oomkill' to appear in seeds/events/related — "
            "the OOM signal was lost"
        )
...
class TestStep7RemediationEngine:
    ...
    def test_at_least_one_hypothesis(self, hypotheses):
        assert hypotheses, "RemediationEngine produced no hypotheses for case 003"

    def test_oom_kill_rule_fires(self, hypotheses):
        """oom_kill rule must fire — OOMKilled exit code 137 is present."""
        rule_ids = [h.rule_id for h in hypotheses]
        assert "oom_kill" in rule_ids, (
            f"oom_kill rule did not fire. Fired rules: {rule_ids}"
        )

    def test_helm_drift_rule_fires(self, hypotheses):
        """helm_drift rule must fire — memory limit drift (512Mi → 50Mi) is present."""
        rule_ids = [h.rule_id for h in hypotheses]
        assert "helm_drift" in rule_ids, (
            f"helm_drift rule did not fire. Fired rules: {rule_ids}"
        )

    def test_oom_kill_weight_highest(self, hypotheses):
        """oom_kill (base 0.88) should be the top-weighted hypothesis."""
        top = hypotheses[0]
        assert top.rule_id == "oom_kill", (
            f"Expected oom_kill as top hypothesis, got: {top.rule_id} w={top.weight:.2f}"
        )

    def test_oom_kill_commands_contain_kubectl(self, hypotheses):
        h = next(h for h in hypotheses if h.rule_id == "oom_kill")
        cmds_text = " ".join(h.commands).lower()
        assert "kubectl" in cmds_text

    def test_oom_kill_commands_reference_namespace(self, hypotheses):
        h = next(h for h in hypotheses if h.rule_id == "oom_kill")
...
class TestStep8PromptDryRun:
    ...
    def test_prompt_contains_oomkilled(self, prompt):
        assert "OOMKilled" in prompt or "oomkilled" in prompt.lower()

    def test_prompt_contains_memory_signal(self, prompt):
        """The memory limit drift must be visible to the LLM."""
        assert "memory" in prompt.lower() or "limit" in prompt.lower(), (
            "Memory limit signal missing from prompt — LLM won't see the root cause"
        )

    def test_prompt_contains_anchor_info(self, prompt):
        """Declared memory anchor must appear — helps LLM identify config drift."""
        assert "anchor" in prompt.lower() or "declared" in prompt.lower() or "512" in prompt, (
...
class TestStep9Proposals:
    ...
    def dry_run_report(self, graph, ctx):
        ...
        if top:
            raw = (
                ...
                f"### 3. Root cause\n{top.explanation}\n\n"
                f"### 4. Causal chain\n"
                + "\n".join(f"{i+1}. {ev}" for i, ev in enumerate(top.evidence or ["OOMKilled exit=137"]))
                + "\n\n### 5. Remediation\n"
    ...
    def test_resource_or_memory_proposal_present(self, proposals):
        """
        A 'resource' or 'memory' related proposal must appear — OOMKilled signals
        a memory limit issue, so ProposalEngine should propose a resource check.
        """
        categories = [p.category for p in proposals]
        all_text = " ".join(
            p.description + " " + p.follow_up_query for p in proposals
        ).lower()
        assert "resource" in categories or "memory" in all_text or "limit" in all_text, (
            f"Expected resource/memory proposal (OOM check). Got categories: {categories}"
        )

    def test_generic_logs_proposal_present(self, proposals):
        """Container logs proposal must fire — OOMKilled/restart signals."""
        all_desc = " ".join(p.description + " " + p.follow_up_query for p in proposals).lower()
        assert "log" in all_desc or "restart" in all_desc or "oom" in all_desc, (
            f"No logs/restart/OOM proposal found.\nProposals: {[(p.category, p.description) for p in proposals]}"
        )
